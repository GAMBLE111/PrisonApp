...
def parse(filepath: str) -> List[Dict[str, Any]]:
    """Parse the Corrective Services Excel file into a list of records."""
    ...
    for sheet_name in wb.sheetnames:
        ...
        for i, row in enumerate(rows[:20]):
            ...
            if "custody" in row_str or "correction" in row_str or "quarter" in row_str:
                header_row = row
                header_idx = i
                break
...
def _map_columns(header_row) -> Dict[str, int]:
    ...
    for i, cell in enumerate(header_row):
        ...
        elif "order" in name or "type" in name:
            col_map["orderType"] = i
