...
def parse(filepath: str) -> List[Dict[str, Any]]:
    """Parse the Prisoners Excel file into a list of records."""
    ...
    for sheet_name in wb.sheetnames:
        ...
        for i, row in enumerate(rows[:20]):
            ...
            if "offence" in row_str or "prisoner" in row_str or "sentence" in row_str:
                header_row = row
                header_idx = i
                break
...
def _map_columns(header_row) -> Dict[str, int]:
    ...
    for i, cell in enumerate(header_row):
        ...
        if "state" in name or "territory" in name:
            ...
            col_map["ageGroup"] = i
        elif "sentence" in name and "length" in name:
            col_map["sentenceLength"] = i
        elif "legal" in name or "status" in name:
            col_map["legalStatus"] = i
